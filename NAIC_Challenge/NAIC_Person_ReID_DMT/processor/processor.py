...
def do_train(cfg,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid) in enumerate(train_loader):
            ...
            # # model.eval()
            # for n_iter, (img, pid, camid, imgpath) in enumerate(val_loader_normal):
            #     img = img.to(device)
            #     # print("len PID: ", list(pid))
            #     # target = torch.tensor(list(pid)).to(device)
            #     target = torch.tensor(list(pid)).to(device)
            ...
            #     logger.info("Validation Acc: {:.3f}".format(acc_meter.avg))
        ...
        if epoch % checkpoint_period == 0:
            model_path = os.path.join(cfg.OUTPUT_DIR, cfg.MODEL.NAME + '_{}.pth'.format(epoch))
